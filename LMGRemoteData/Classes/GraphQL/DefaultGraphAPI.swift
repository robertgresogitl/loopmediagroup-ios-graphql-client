...
public struct BusinessListItem: GraphQLFragment {
  ...
  public init(id: GraphQLID, name: String, shortName: String, categories: [Category], profileImages: [ProfileImage]) {
    self.init(unsafeResultMap: ["__typename": "Business", "id": id, "name": name, "shortName": shortName, "categories": categories.map { (value: Category) -> ResultMap in value.resultMap }, "profileImages": profileImages.map { (value: ProfileImage) -> ResultMap in value.resultMap }])
  }
